...
public class User extends Thread {
	...
	public static void main(String[] args) {
		...
		if (userSocket != null && output_stream != null && input_stream != null) {
			try {
				...
				ClientMessage = inputLine.readLine().trim();
				...
				while (!closed) {
						...
						if (ClientMessage.startsWith("@connect")) {
							output_stream.println("#friendme "+ClientMessage.substring(9));
						}
						else if (ClientMessage.startsWith("@friend")) {
							output_stream.println("#friends "+ClientMessage.substring(8));
						}
						else if (ClientMessage.startsWith("@deny")) {
							output_stream.println("#DenyFriendRequest "+ClientMessage.substring(6));
						}
						else if (ClientMessage.startsWith("@disconnect")) {
							output_stream.println("#unfriend "+ClientMessage.substring(12));
						}
	...
	public void run() {
		...
		try {
			//Get Server Response and check if it starts with an acceptable command as per the protocol
			...
			while ((responseLine = input_stream.readLine()) != null) {
				if (responseLine.startsWith("#welcome")) {
					...
					responses = responseLine.split(" ");
					System.out.println(responses[1]+" wants to be your friend");
				}
				else if (responseLine.startsWith("#OKfriends")) {
					responses = responseLine.split(" ");
					System.out.println("You are now friends with: "+responses[1]);
				}
				else if (responseLine.startsWith("#FriendRequestDenied")) {
					responses = responseLine.split(" ");
					System.out.println(responses[1]+" has rejected your friend request");
				}
				else if (responseLine.startsWith("#NotFriends")) {
					responses = responseLine.split(" ");
					System.out.println("You are no longer friends with: "+responses[1]);
				}
